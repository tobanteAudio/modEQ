  ...
  , eqProcessor(state)
{
  ...
  for (int i = 0; i < int(bands.size()); ++i)
  {
    ...
    state.createAndAddParameter(
      eqProcessor.getTypeParamName(i), band.name + " Type", TRANS("Filter Type"),
    ...
    state.createAndAddParameter(
      eqProcessor.getFrequencyParamName(i), band.name + " freq", "Frequency",
      NormalisableRange<float>(20.0, 20000.0, 1.0), band.frequency,
      ...
      false, true, false);
    state.createAndAddParameter(eqProcessor.getQualityParamName(i), band.name + " Q", TRANS("Quality"),
    ...
    state.createAndAddParameter(
      eqProcessor.getGainParamName(i), band.name + " gain", TRANS("Gain"),
      NormalisableRange<float>(1.0f / maxGain, maxGain, 0.001f), band.gain,
      [](float value) { return String(Decibels::gainToDecibels(value), 1) + " dB"; },
      [](String text) { return Decibels::decibelsToGain(text.dropLastCharacters(3).getFloatValue()); }, false, true,
      false);
    state.createAndAddParameter(eqProcessor.getActiveParamName(i), band.name + " active", TRANS("Active"),
    ...
    state.addParameterListener(eqProcessor.getTypeParamName(i), this);
    state.addParameterListener(eqProcessor.getFrequencyParamName(i), this);
    state.addParameterListener(eqProcessor.getQualityParamName(i), this);
    state.addParameterListener(eqProcessor.getGainParamName(i), this);
    state.addParameterListener(eqProcessor.getActiveParamName(i), this);
  }
...
void FrequalizerAudioProcessor::prepareToPlay(double newSampleRate, int newSamplesPerBlock)
{
  ...
  filter.get<6>().setGainLinear(*state.getRawParameterValue(TA::EqualizerProcessor::paramOutput));
...
void FrequalizerAudioProcessor::parameterChanged(const String& parameter, float newValue)
{
  ...
  for (size_t i = 0; i < bands.size(); ++i)
  {
    if (parameter.startsWith(getBandName(int(i)) + "-"))
    {
      ...
      {
        bands[i].type = static_cast<TA::EqualizerProcessor::FilterType>(static_cast<int>(newValue));
      }
...
bool FrequalizerAudioProcessor::getBandSolo(const int index) const { return index == soloed; }

TA::EqualizerProcessor::Band* FrequalizerAudioProcessor::getBand(const int index)
{
  if (isPositiveAndBelow(index, bands.size()))
...
const std::vector<double>& FrequalizerAudioProcessor::getMagnitudes() { return magnitudes; }

void FrequalizerAudioProcessor::createFrequencyPlot(Path& p,
