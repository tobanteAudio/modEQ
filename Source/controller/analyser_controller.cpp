...
AnalyserController::AnalyserController(
    tobanteAudio::EqualizerProcessor& p,
    ...
    : processor(p), bandControllers(bc), view(v)
{
    ...
    {
        ignoreUnused(band);
        view.handles.emplace_back(
            tobanteAudio::AnalyserView::BandHandle {i, 0, 0, 0, 0});
        ++i;
    }
...
void AnalyserController::timerCallback()
{
    ...
    {
        processor.createAnalyserPlot(view.in_analyser, view.plotFrame, 20.0f,
                                     true);
        processor.createAnalyserPlot(view.out_analyser, view.plotFrame, 20.0f,
                                     false);
...
void AnalyserController::mouseDown(const MouseEvent& e)
{
    ...
    if (e.mods.isPopupMenu() && plotFrame.contains(e.x, e.y))
    {
        for (int i = 0; i < bandControllers.size(); ++i)
        {
            ...
            if (view.overlap_with_radius(pos, e.position.getX(),
                                         HANDLE_CLICK_RADIUS))
            {
                auto& contextMenu = view.contextMenu;
                contextMenu.clear();
                for (int t = 0;
                     t < tobanteAudio::EqualizerProcessor::LastFilterID; ++t)
                    contextMenu.addItem(
                        t + 1,
                        tobanteAudio::EqualizerProcessor::getFilterTypeName(
                            static_cast<
                                tobanteAudio::EqualizerProcessor::FilterType>(
                                t)),
                        true, band->type == t);

                contextMenu.showMenuAsync(
                    PopupMenu::Options()
                        .withTargetComponent(&view)
                        .withTargetScreenArea(
                            {e.getScreenX(), e.getScreenY(), 1, 1}),
                    [this, i](int const selected) {
                        if (selected > 0)
                            bandControllers.getUnchecked(i)->setType(selected
                                                                     - 1);
                    });
            }  // If mouse x overlaps
        }      // For all bands
        ...
        for (int i = 0; i < bandControllers.size(); ++i)  //
        {
                                         ...
                                         HANDLE_CLICK_RADIUS))
            {
                ...
                if (view.overlap_with_radius(gainPos, e.position.getY(),
                                             HANDLE_CLICK_RADIUS))
                {
                    draggingGain = processor.state.getParameter(
                        processor.getGainParamID(i));
                    view.setMouseCursor(
...
void AnalyserController::mouseDrag(const MouseEvent& e)
{
    ...
    if (isPositiveAndBelow(draggingBand, bandControllers.size()))
    {
        ...
        bandControllers[draggingBand]->setFrequency(
            view.get_frequency_for_position(pos));
        if (draggingGain)
    ...
    if (plotFrame.contains(e.x, e.y))
    {
        for (int i = 0; i < bandControllers.size(); ++i)
        {
            ...
            if (view.overlap_with_radius(pos, e.position.getX(),
                                         HANDLE_CLICK_RADIUS))
            {
                if (auto* param = processor.state.getParameter(
                        processor.getActiveParamID(i)))
                {
                                        ...
                                        const MouseWheelDetails& wheel)
{
    ...
    if (plotFrame.contains(event.x, event.y))
    {
        ...
        for (int i = 0; i < bandControllers.size(); ++i)
        {
            ...
            // If mouse & band match on x-axis
            if (view.overlap_with_radius(pos, event.position.getX(),
                                         HANDLE_CLICK_RADIUS))
            {
                const auto frameY = static_cast<float>(plotFrame.getY());
                const auto frameBottom
...
void AnalyserController::updateFrequencyResponses()
{
    ...
    processor.createFrequencyPlot(view.frequencyResponse,
                                  processor.getMagnitudes(), plotFrame,
                                  pixelsPerDouble);
}
}  // namespace tobanteAudio